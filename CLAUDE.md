...
- `getDb(config)` returns a cached client (LRU, max 10) with SSRF host validation
  ...
  The return type is the structural `DbClient` exported from `src/lib/db.ts`
  (typed model delegates + interactive and batch `$transaction`) — routes use
  it directly; do not reintroduce `(db as any)` casts.
...
9. **React Query provider lives in the layout, not in pages** — `src/app/providers.tsx`
   (`Providers`, mounted in `src/app/layout.tsx`) wraps `{children}` so every page hook sits
   below the `QueryClientProvider`. Never render a provider inside a page's own JSX: a hook
   in the same component cannot see it (context flows to descendants only) — that caused the
   "No QueryClient set" 500 on `GET /` after the phase-8 React Query migration. The shared
   query hooks (`useJiraPollQuery`, `usePluginEventsQuery`, `useMasterDatasetQuery`) are also
   SSR-guarded (`typeof window !== 'undefined'`) since they hit client-only endpoints.
...
- **E2E (Playwright)** — specs in `e2e/` (`npm run e2e`), driven against the dev server
  ...
  **E2E is a required gate after any change to the frontend data layer** (providers,
  hooks, React Query wiring) or anything in `src/app/` — unit tests always wrap renders
  in a provider, so provider-placement/SSR regressions only surface at page-load level
  (see the 8-hotfix "No QueryClient set" incident in `docs/DEBT_CLEANUP.md`).
