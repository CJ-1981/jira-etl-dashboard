 ...
 * @MX:REASON: This endpoint is unauthenticated and destructively deletes all
 * data for a connection. Any website the user visits could send a `no-cors`
 * request to the localhost server. Browsers attach an `origin` (or
 * `referer`) header to such cross-origin requests, so we reject every request
