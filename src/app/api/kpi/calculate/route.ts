 ...
 * @MX:REASON: This endpoint is unauthenticated and accepts custom plugin
 * formulas. Any website the user visits could POST a `no-cors` text/plain
 * request to the localhost server. Browsers attach an `origin` (or
 * `referer`) header to such cross-origin POSTs, so we reject every request
 * whose origin is not a loopback address. Requests without these headers
 * (server-side fetches, curl, Electron main process) pass through.
 * @MX:NOTE: Keep this implementation identical to the one in /api/jira/extract.
 */
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // @MX:NOTE: When dateFrom is missing, default to a rolling 90-day window ending at `end`
    // instead of a hardcoded calendar date (previously 2024-01-01), which silently grew
    // stale and produced misleading KPI periods.
