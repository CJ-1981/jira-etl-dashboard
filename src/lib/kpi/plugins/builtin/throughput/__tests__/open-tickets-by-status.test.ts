...
describe('open_tickets_by_status Plugin', () => {
  ...
  it('should calculate open tickets grouped by status', () => {
    ...
    const inProgressResult = results.find((r: any) => r.name === 'Status: In Progress');
    ...
    const toDoResult = results.find((r: any) => r.name === 'Status: To Do');
    ...
    const unassignedResult = results.find((r: any) => r.name === 'Status: Unassigned');
  ...
  it('should sort results by count descending', () => {
    ...
    expect(results[0].name).toBe('Status: In Progress'); // 3 tickets
    expect(results[1].name).toBe('Status: To Do'); // 2 tickets
  });
});
